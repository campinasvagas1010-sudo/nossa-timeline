...
/**
 * API de teste - Cria uma story premium de teste
 * GET /api/test/create-premium
 */
export async function GET() {
  try {
    // Deletar story de teste anterior
    await supabaseAdmin
      .from('stories')
      .delete()
      .eq('slug', 'test-premium');

    // Criar nova story premium de teste (apenas campos essenciais)
    const testMoments = [
          {
            title: 'O Primeiro Oi',
            emoji: '✨',
    ...
    if (error) {
      ...
      success: true,
      message: 'Story premium de teste criada!',
      story: data,
      test_url: `${process.env.NEXT_PUBLIC_APP_URL || 'http://localhost:3000'}/h/test-premium`,
      expires_at: data.expires_at,
      instructions: [
        '1. Acesse: http://localhost:3000/h/test-premium',
        '2. Deve mostrar contador "Expira em 2h"',
        '3. Badge PREMIUM deve aparecer',
        '4. Timeline completa com 15 momentos',
        '5. Botões "Ocultar" em cada momento',
        '6. Clique em "Ocultar" - deve blur o card',
        '7. Preferências salvas no localStorage'
      ]
    });
